...
class ModifyEscaperInputState:
    ...
    def __init__(self, removeIndex, xBoundary, yBoundary):
        self.removeIndex = removeIndex
        self.xBound = xBoundary
        self.yBound = yBoundary

    def __call__(self, worldState):
        agentState = [np.delete(state, self.removeIndex) for state in worldState]
        distance = [
            [abs(state[0] - self.xBound[0]), abs(state[0] - self.xBound[1]),
             abs(state[1] - self.yBound[0]), abs(state[1] - self.yBound[1])] for
            state in agentState]
        newAgentState = [np.concatenate([agentState[index], distance[index]])
                         for index in range(len(agentState))]
...
class EvaluateEscaperPerformance:
    ...
    def __init__(self, chaserPolicy, allSampleTrajectory, measure,
                 ...
                 getPreparePolicy, getModelSavePath, modifyEscaperState):
        ...
        self.modifyEscaperState = modifyEscaperState
    ...
    def __call__(self, df):
        ...
        extraStateNum, addInfoFunction = extraInfo
        ...
        generateEscaperModel = self.getGenerateEscaperModel(numOfStateSpace+extraStateNum)
        ...
        preparePolicy = self.getPreparePolicy(self.modifyEscaperState)
...
def main():
    ...
    independentVariables['extraInfo'] = [(8, 'toBoundDistance')]
    ...
    xBound = [-10, 10]
    yBound = [-10, 10]
    modifyEscaperState = ModifyEscaperInputState(qPosIndex, xBound, yBound)
